...
export default function WrittenUpdateForm(props) {
  const {
    id: idProp,
    written: writtenModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = writtenRecord
      ? { ...initialValues, ...writtenRecord }
      : initialValues;
  ...
  const [writtenRecord, setWrittenRecord] = React.useState(writtenModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Written, idProp)
        : writtenModelProp;
      setWrittenRecord(record);
    };
    queryData();
  }, [idProp, writtenModelProp]);
  React.useEffect(resetStateValues, [writtenRecord]);
  const validations = {
    ownerID: [],
    props: [{ type: "JSON" }],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Written.copyOf(writtenRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WrittenUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || writtenModelProp)}
