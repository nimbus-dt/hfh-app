...
export default function HabitatUpdateForm(props) {
  const {
    id: idProp,
    habitat: habitatModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = habitatRecord
      ? { ...initialValues, ...habitatRecord }
      : initialValues;
    setName(cleanValues.name);
    setUrlName(cleanValues.urlName);
    ...
    setAMI(cleanValues.AMI ?? []);
    setCurrentAMIValue("");
    setErrors({});
  };
  const [habitatRecord, setHabitatRecord] = React.useState(habitatModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Habitat, idProp)
        : habitatModelProp;
      setHabitatRecord(record);
    };
    queryData();
  }, [idProp, habitatModelProp]);
  React.useEffect(resetStateValues, [habitatRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Habitat.copyOf(habitatRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "HabitatUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || habitatModelProp)}
