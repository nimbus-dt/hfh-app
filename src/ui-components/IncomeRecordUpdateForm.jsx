...
export default function IncomeRecordUpdateForm(props) {
  const {
    id: idProp,
    incomeRecord: incomeRecordModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = incomeRecordRecord
      ? { ...initialValues, ...incomeRecordRecord }
      : initialValues;
    setOwnerID(cleanValues.ownerID);
    setTypeOfIncome(cleanValues.typeOfIncome);
  ...
  const [incomeRecordRecord, setIncomeRecordRecord] = React.useState(
    incomeRecordModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(IncomeRecord, idProp)
        : incomeRecordModelProp;
      setIncomeRecordRecord(record);
    };
    queryData();
  }, [idProp, incomeRecordModelProp]);
  React.useEffect(resetStateValues, [incomeRecordRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            IncomeRecord.copyOf(incomeRecordRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "IncomeRecordUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || incomeRecordModelProp)}
