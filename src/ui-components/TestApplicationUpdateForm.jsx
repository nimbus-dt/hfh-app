...
export default function TestApplicationUpdateForm(props) {
  const {
    id: idProp,
    testApplication: testApplicationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = testApplicationRecord
      ? { ...initialValues, ...testApplicationRecord }
      : initialValues;
    setOwnerID(cleanValues.ownerID);
    setLastSection(cleanValues.lastSection);
    setSubmittedDate(cleanValues.submittedDate);
    setReviewStatus(cleanValues.reviewStatus);
    setSubmissionStatus(cleanValues.submissionStatus);
    setProps(
  ...
  const [testApplicationRecord, setTestApplicationRecord] = React.useState(
    testApplicationModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TestApplication, idProp)
        : testApplicationModelProp;
      setTestApplicationRecord(record);
    };
    queryData();
  }, [idProp, testApplicationModelProp]);
  React.useEffect(resetStateValues, [testApplicationRecord]);
  const validations = {
    ownerID: [],
    lastSection: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TestApplication.copyOf(testApplicationRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TestApplicationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || testApplicationModelProp)}
