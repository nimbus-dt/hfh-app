...
export default function DebtUpdateForm(props) {
  const {
    id: idProp,
    debt: debtModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = debtRecord
      ? { ...initialValues, ...debtRecord }
      : initialValues;
  ...
  const [debtRecord, setDebtRecord] = React.useState(debtModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Debt, idProp)
        : debtModelProp;
      setDebtRecord(record);
    };
    queryData();
  }, [idProp, debtModelProp]);
  React.useEffect(resetStateValues, [debtRecord]);
  const validations = {
    ownerId: [],
    props: [{ type: "JSON" }],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Debt.copyOf(debtRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DebtUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || debtModelProp)}
