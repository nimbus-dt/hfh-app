...
export default function ApplicantOptionalUpdateForm(props) {
  const {
    id: idProp,
    applicantOptional: applicantOptionalModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = applicantOptionalRecord
      ? { ...initialValues, ...applicantOptionalRecord }
      : initialValues;
  ...
  const [applicantOptionalRecord, setApplicantOptionalRecord] = React.useState(
    applicantOptionalModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ApplicantOptional, idProp)
        : applicantOptionalModelProp;
      setApplicantOptionalRecord(record);
    };
    queryData();
  }, [idProp, applicantOptionalModelProp]);
  React.useEffect(resetStateValues, [applicantOptionalRecord]);
  const validations = {
    ownerID: [],
    props: [{ type: "JSON" }],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ApplicantOptional.copyOf(applicantOptionalRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ApplicantOptionalUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || applicantOptionalModelProp)}
