...
export default function ApplicantInfoUpdateForm(props) {
  const {
    id: idProp,
    applicantInfo: applicantInfoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = applicantInfoRecord
      ? { ...initialValues, ...applicantInfoRecord }
      : initialValues;
  ...
  const [applicantInfoRecord, setApplicantInfoRecord] = React.useState(
    applicantInfoModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ApplicantInfo, idProp)
        : applicantInfoModelProp;
      setApplicantInfoRecord(record);
    };
    queryData();
  }, [idProp, applicantInfoModelProp]);
  React.useEffect(resetStateValues, [applicantInfoRecord]);
  const validations = {
    ownerID: [],
    props: [{ type: "JSON" }],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ApplicantInfo.copyOf(applicantInfoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ApplicantInfoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || applicantInfoModelProp)}
